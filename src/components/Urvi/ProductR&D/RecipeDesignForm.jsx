...
const RecipeForm = () => {
  ...
  const handlesubmit = async () => {
    ...
    if (recipeName === "" || ingredientsList.length === 0) {
      ...
      setResponse({ message: "Enter All Fields", status: "error" });
      return;
    }
    const response = await AddToIngredentProfile(bodyContent);
    if (response.status === 401) {
      setResponse({ message: "Change Ingredient Name", status: "error" });
      return;
    } else {
      setResponse({
    ...
    if (response2.status === 401) {
      setResponse({ message: "Recipe Name  Already Exists", status: "error" });
      return;
    }
    ...
    <>
      ...
      <div className="formcontains">
        <div className="recipeform_buttons_options">
          <button
            id={ingredientProfile ? "active" : ""}
            onClick={() => setIngredientProfile(!ingredientProfile)}
          >
            Ingredient Profile
          </button>
