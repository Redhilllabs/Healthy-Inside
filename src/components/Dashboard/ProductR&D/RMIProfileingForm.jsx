...
import { getmateriallist, addtomateriallist } from "../../../utils/ApiCall";
...
const RMIProfileingForm = () => {
  ...
  const handelrmisubmit = async (event) => {
    ...
    if (Object.values(bodyContent).some((value) => value === "")) {
      setResponse({ message: "Please fill all fields", status: "error" });
    } else {
      const response = await addtomateriallist(JSON.stringify(bodyContent));
      if (response.status === 401) {
        setResponse({ message: "This Ingredient already exists", status: "error" });
        return;
      } else {
        setResponse({
      ...
      <div className="formcontains">
        ...
        {kitchenOption === "AddToList" && (
          <form class="form" id="RMIform-designing" onSubmit={handelrmisubmit}>
            ...
            <input id="rmiaddtolist" type="submit" value="Add to List"></input>
          </form>
        )}
      </div>
      ...
      {showtable && (
        <div className="table-container" id="showtablermi">
          ...
          <div id="tabel_controllers">
            ...
            <div id="recipebutton_save" onClick={handelrmisubmit}>
              Submit
            </div>
          </div>
        </div>
