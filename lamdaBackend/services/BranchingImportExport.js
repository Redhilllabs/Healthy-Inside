...
const BranchingTable = "BranchingImportAndExport"
...
async function searchBranchingImportExport(data) {
  ...
  for (const item of SalesPlanList) {
    ...
    for (const name in user.ItemList) {
      const branchingUser = await getBranchingImportExportUser(user.ItemList[name].Constituent_Recipe);
      const ExtrabatchingUser = await getExtraBatching(user.ItemList[name].Constituent_Recipe);
      
      if (branchingUser) {
        for (const branch of branchingUser) {
          const importSupply = [];
          const exportSupply = [];
...
async function getBranchingImportExportUser(e) {
  const params = {
    TableName: BranchingTable,
...
module.exports = {searchBranchingImportExport};